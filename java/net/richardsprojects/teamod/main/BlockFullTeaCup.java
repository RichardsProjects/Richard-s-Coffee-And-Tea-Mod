...
public class BlockFullTeaCup extends BlockContainer {
	
	protected BlockFullTeaCup(Material mat)
	{
		super(mat);
	...
	{
		fullTeaCup = new BlockFullTeaCup(Material.wood).setBlockName("fullTeaCup").setBlockTextureName("iron_block").setHardness(1F).setCreativeTab(null);
	}
	
	public static void registerBlock()
	{
		GameRegistry.registerBlock(fullTeaCup, fullTeaCup.getUnlocalizedName());
	}
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int idk1, float idk2, float idk3, float idk4)
    {
		...
		else
		{
			if(!world.isRemote)
			{
				...
				world.setBlock(x, y, z, BlockHalfTeaCup.halfTeaCup, metadata, 3);
	...
	{
		return new ItemStack(ItemTeaCup.fullTeaCup, 1);
	}
